...
from models import TargetNet, MultiHeadNet, JointHeadNet, WeightedUnet3D, SiameseWeightedUnet3D, TargetNetV2
...
def train(train_loader, val_loader, target_model, optimizer, scheduler, criterion):
    train_losses = []
    valid_losses = []
    avg_train_losses = []
    avg_valid_losses = []
    ...
    for epoch in range(intial_epoch, nb_epoch):
        ...
        with torch.no_grad():
             for batch_ndx, (x_pre, y) in enumerate(val_loader):
                 ...
                 if (batch_ndx + 1) % 3 ==0:
                     print('Epoch [{}/{}], batch_ndx {}, Val Loss: {:.6f}'.format(epoch + 1, conf.nb_epoch, batch_ndx + 1, np.average(valid_losses)))
                     sys.stdout.flush()
                 total += y.size(0)
...
if __name__ == "__main__":
  ...
  #base_model = denseUnet3d.UNet3DClassification()
  #base_model = unet3d.UNet3DClassificationDense()

  #base_model = NestedUNet()
  ...
  #target_model = loadEncoder(target_model, '/data/yyang409/unath/pretrained_weights/classification/kras-dense-unet/83.pt')
  #target_model = loadEncoder("/data/yyang409/unath/pretrained_weights/classification/kras-unet-multi/13.pt")
  #model_path = "/data/yyang409/unath/pretrained_weights/classification/kras-unet-multi/14.pt"
  model_path = "/data/yyang409/unath/pretrained_weights/classification/kras-unet-standard-v5/27.pt"
  ...
  valiate(target_model, val_loader)
  #test(target_model, model_path, val_loader)
